#include "CalibratorMulticam.h"
#include <opencv2/calib3d.hpp>
#include <iomanip>

namespace ecv {
    std::tuple<cv::Mat, cv::Mat, double> CalibratorMulticam::getStereoUndistortMap(cv::Size frameSize, const CalibrationData &base, const CalibrationData &current, double alpha) {
        cv::Rect roi1, roi2;
...
//        );
    ...
    void CalibratorMulticam::convertTo2dPoints(const std::vector<cv::Point3d> &points3d, std::vector<cv::Point2f> &points2d) {
        for (int j = 0; j < points2d.size(); ++j) {
            points2d[j] = cv::Point(points3d[j].x, points3d[j].y);
        }
    }

    void CalibratorMulticam::convertToPlain3dPoints(const std::vector<cv::Point3d> &points1, std::vector<cv::Point3f> &points2) {
        for (int j = 0; j < points2.size(); ++j) {
            points2[j] = cv::Point3f(points1[j].x, points1[j].y, 0);
        }
    }

    void CalibratorMulticam::convertTo2dPoints(const std::vector<cv::Point3f> &points3d, std::vector<cv::Point2f> &points2d) {
        for (int j = 0; j < points2d.size(); ++j) {
            points2d[j] = cv::Point(points3d[j].x, points3d[j].y);
        }
    }

    double CalibratorMulticam::calibrateCameraPair(
            cv::Size frameSize,
     ...
     */
    void CalibratorMulticam::printStereoCalibrationStats(const cv::Mat& camMatrixL, const cv::Mat& camMatrixR,
